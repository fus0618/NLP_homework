...
class ESimcseModel(nn.Module):
    ...
    def __init__(self, pretrained_model, pooling, dropout=0.3, off_dropout=False):
        super(ESimcseModel, self).__init__()
        config = BertConfig.from_pretrained(pretrained_model)
        config.attention_probs_dropout_prob = dropout  # 修改config的dropout系数
        config.hidden_dropout_prob = dropout
        self.bert = BertModel.from_pretrained(pretrained_model, config=config)
        self.pooling = pooling
        self.off_dropout = off_dropout  # 新增参数，控制off_dropout

    def forward(self, input_ids, attention_mask, token_type_ids):
        if self.off_dropout:
            self.bert.eval()  # 禁用dropout
        else:
            self.bert.train()  # 启用dropout（默认行为）
